...
$result = mysql_query("SELECT category FROM category where sid=$sid");
$data = array();
...
$result2 = mysql_query("SELECT chapter FROM chapter where sid=$sid");
$data2 = array();
while ($row = mysql_fetch_array($result2)) {
    array_push($data2, $row["chapter"]);
}
 ?>
...
<html>
 <head>
  ...
  <script>
	 ...
	 var columns = new Array("qtype","category","chapter","sid");
	 var placeholder = new Array("","","","Above mentioned");
	 var inputType = new Array("select","select1","select2","text");
	 var table = "tableDemo";
	 var selectOpt = new Array("Long","Short","Very Short");
	 var selectOpt2 = <?php echo json_encode($data, JSON_PRETTY_PRINT) ?>;
	 var selectOpt3 = <?php echo json_encode($data2, JSON_PRETTY_PRINT) ?>;
	 ;
