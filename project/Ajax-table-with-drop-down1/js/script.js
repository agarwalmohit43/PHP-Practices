...
createInput = function(i,str){
	...
	else if(inputType[i] == "select1"){
		input = '<select name='+columns[i]+'>';
		for(i=0;i<selectOpt2.length;i++){
			//console.log(selectOpt[i]);
			selected = "";
			if(str == selectOpt2[i])
				selected = "selected";
			input += '<option value="'+selectOpt2[i]+'" '+selected+'>'+selectOpt2[i]+'</option>';
		}
		input += '</select>';
		//console.log(str);
	}
	
	else if(inputType[i] == "select2"){
		input = '<select name='+columns[i]+'>';
		for(i=0;i<selectOpt3.length;i++){
			//console.log(selectOpt[i]);
			selected = "";
			if(str == selectOpt3[i])
				selected = "selected";
			input += '<option value="'+selectOpt3[i]+'" '+selected+'>'+selectOpt3[i]+'</option>';
		}
		input += '</select>';
		//console.log(str);
	}
