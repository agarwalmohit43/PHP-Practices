...
class ajax_table {
  ...
  function getRecords($sid){
	$this->res = mysql_query("select q.qtype,c.category,ch.chapter from question q,category c,chapter ch where q.category=c.id and q.chapter=ch.id and c.sid=$sid");
  ...
  function save($data){
	if(count($data)){
		...
		mysql_query("insert into question (".implode(",",array_keys($data)).") values ('".$values."')");
  ...
  function delete_record($id){
	 if($id){
		mysql_query("delete from question where id = $id limit 1");
  ...
  function update_record($data){
	if(count($data)){
		...
		$str = substr($str,0,-1);
		$sql = "update question set $str where id = $id limit 1";
  ...
  function update_column($data){
	if(count($data)){
		$id = $data['rid'];
		unset($data['rid']);
		$sql = "update question set ".key($data)."='".$data[key($data)]."' where id = $id limit 1";
		$res = mysql_query($sql);
		if(mysql_affected_rows()) return $id;
		return 0;
