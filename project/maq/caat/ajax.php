<?php
	...
	if(isset($_POST) && count($_POST)){
		...
		if($action == "save"){		
			...
			$sql="SELECT c.category,ch.chapter FROM category as c,chapter as ch where c.id=".$escapedPost['category']." and ch.id=".$escapedPost['chapter'];
			$querys=mysql_query($sql);
			$row=mysql_fetch_assoc($querys);

			$escapedPost['category']=$row["category"];
			$escapedPost['chapter']=$row["chapter"];
